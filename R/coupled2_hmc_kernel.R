#' @rdname coupled2_hmc_kernel
#' @title HMC kernel for two coupled chains
...
coupled2_hmc_kernel <- function(level, state1, state2, identical, tuning, proposal_coupling){

  cost = 0  # numerilal cost of calculations
  ...
  if (proposal_value$identical){
    ...
    proposal_velocity2 <- proposal_velocity1
    initial_velocity2 <- initial_velocity1
