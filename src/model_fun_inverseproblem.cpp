...
double compute_alpha_inverseproblem(NumericMatrix mle, NumericVector d, NumericVector r)
{
    ...
    if(inner_product_inverseproblem(y, y) > eps)
    {
        alpha = inner_product_inverseproblem(r, r) / inner_product_inverseproblem(y, y);
    }
    return alpha;
}
...
double compute_beta_inverseproblem(NumericMatrix mle, NumericVector d, NumericVector rcg)
{
    ...
    if(inner_product_inverseproblem(y0, y0) > eps)
    {
        beta = inner_product_inverseproblem(y0, y1) / inner_product_inverseproblem(y0, y0);
    }
    return beta;
}
...
NumericVector linear_solver_inverseproblem(NumericMatrix mle, NumericVector b)
{
    ...
    umat = sparse_u_factor_inverseproblem(mle);
    lmat = sparse_l_factor_inverseproblem(mle);
    z = inverse_l_vec_inverseproblem(lmat, b);
    for(k0 = 0; k0 < n; k0++)
    {
        r[k0] = z[k0];
        d[k0] = 0.0;
    ...
    {
        y = inverse_u_vec_inverseproblem(umat, x);
        y = sparse_mat_vec_mult_inverseproblem(mle, y);
        y = inverse_l_vec_inverseproblem(lmat, y);
        for(k0 = 0; k0 < n; k0++)
        {
            r[k0] = z[k0] - y[k0];
        }

        d = inverse_u_vec_inverseproblem(umat, r);
        d = sparse_mat_vec_mult_inverseproblem(mle, d);
        d = inverse_l_vec_inverseproblem(lmat, d);

        factor = 1.0 * inner_product_inverseproblem(d, r) / (inner_product_inverseproblem(d, d) + eps);
        for(k0 = 0; k0 < n; k0++)
        {
            x[k0] += factor * r[k0];
        }
        res = sqrt(inner_product_inverseproblem(r, r));
        //cout << "residual = " << res << endl;
    }
    if(res > 1.0e-3)
    {
        cout << "wraning: high residual = " << res << endl;
    }
    return inverse_u_vec_inverseproblem(umat, x);
}
...
NumericVector cg_solver_inverseproblem(NumericMatrix mle, NumericVector b)
{
    ...
    for(kiter = 0; kiter < niter; kiter++)
    {
        ...
        factor = 1.0 * mat_inner_product_inverseproblem(mle, r, r) / (inner_product_inverseproblem(d, d) + eps);
        for(k0 = 0; k0 < n; k0++)
        {
            x[k0] += factor * r[k0];
        }
    ...
    for(kiter = 0; kiter < niter; kiter++)
    {
        ...
        alpha = compute_alpha_inverseproblem(mle, d, rcg);
        ...
        beta = compute_beta_inverseproblem(mle, d, rcg);
        for(k0 = 0; k0 < n; k0++)
        {
...
NumericMatrix init_system_matrix_inverseproblem(NumericVector param, int nx)
{
    ...
    int nml = 0;
    double pi_num=3.1415926, h = 1.0 / (nx - 1.0);
    double term0 = 0.0, term1 = 0.0;
    nml = compute_matrix_dimension_inverseproblem(nx);
    ...
    for(kx = 1; kx < (nx - 1); kx++)
    {
        ...
        mle(kml, 0) = i0;
        mle(kml, 1) = i1;
        term0 = (cos(pi_num * (kx - 1) * h) - cos(pi_num * (kx + 1) * h)) / 10.0 / pi_num / h;
        term1 = (sin(2.0 * pi_num * (kx + 1) * h) - sin(2.0 * pi_num * (kx - 1) * h)) / 80.0 / pi_num / h;
        mle(kml, 2) = 2.0 * 0.15 + param[0] * term0 + param[1] * term1;
        kml = kml + 1;
        ...
        mle(kml, 0) = i0;
        mle(kml, 1) = i1;
        term0 = (cos(pi_num * kx * h) - cos(pi_num * (kx + 1) * h)) / 10.0 / pi_num / h;
        term1 = (sin(2.0 * pi_num * (kx + 1) * h) - sin(2.0 * pi_num * kx * h)) / 80.0 / pi_num / h;
        mle(kml, 2) = 0.0 - 1.0 * 0.15 - param[0] * term0 - param[1] * term1;
        kml = kml + 1;
        ...
        mle(kml, 0) = i0;
        mle(kml, 1) = i1;
        term0 = (cos(pi_num * (kx - 1) * h) - cos(pi_num * kx * h)) / 10.0 / pi_num / h;
        term1 = (sin(2.0 * pi_num * kx * h) - sin(2.0 * pi_num * (kx - 1) * h)) / 80.0 / pi_num / h;
        mle(kml, 2) = 0.0 - 1.0 * 0.15 - param[0] * term0 - param[1] * term1;
        kml = kml + 1;
    }
...
{
    int nml = compute_matrix_dimension_inverseproblem(nx);
    NumericVector x(nx);
    NumericVector b(nx);
    NumericMatrix mle(nml, 3);

    mle = init_system_matrix_inverseproblem(param, nx);
    b = init_right_part_inverseproblem(param, nx);
    x = linear_solver_inverseproblem(mle, b);
    return x;
}
...
double l2_norm_inverseproblem(NumericVector param, int l)
{
    ...
    int nx = 0;
    double res = 0.0;
    nx = (int)exp(0.5 * (l + 4) * log(2.0));
    NumericVector x(nx);
    x = numerical_solution_inverseproblem(param, nx);
    NumericVector xnumer(nval);
    NumericVector xexact(nval);
    xnumer = numerical_solution_values_inverseproblem(x, nval);
    xexact = exact_solution_inverseproblem(param, nval);
    for(kval = 0; kval < nval; kval ++)
    {
...
NumericMatrix init_system_matrix_derivative_inverseproblem(NumericVector param, NumericVector pgrad, int nx)
{
    ...
    for(kx = 1; kx < (nx - 1); kx++)
    {
        ...
        term0 = (cos(pi_num * (kx - 1) * h) - cos(pi_num * (kx + 1) * h)) / 10.0 / pi_num / h;
        term1 = (sin(2.0 * pi_num * (kx + 1) * h) - sin(2.0 * pi_num * (kx - 1) * h)) / 80.0 / pi_num / h;
        mle_grad(kml, 2) = pgrad[0] * term0 + pgrad[1] * term1;
        kml = kml + 1;
...
NumericVector numerical_solution_derivative_inverseproblem(NumericVector param, NumericVector pgrad, int nx)
{
    ...
    b = init_right_part_inverseproblem(param, nx);
    bgrad = init_right_part_derivative_inverseproblem(param, pgrad, nx);
    mle = init_system_matrix_inverseproblem(param, nx);
    mle_grad = init_system_matrix_derivative_inverseproblem(param, pgrad, nx);
    //cout << "mle_grad = " << mle_grad << endl;
    x = linear_solver_inverseproblem(mle, b);
    for(kml = 0; kml < nml; kml ++)
    {
    ...
    //cout << "x = " << x << endl;
    //cout << "bgrad = " << bgrad << endl;
    xgrad = linear_solver_inverseproblem(mle, bgrad);
...
NumericVector observation_inverseproblem(NumericVector param, int l)
{
    ...
    double eps=1.0e-3;
    double dh = 1.0;
    nx = 1 + (int)(exp((l + 5) * log(2.0)) + eps);
    dh = dh / (nx - 1);
    NumericVector xvec(nvec);
    NumericVector xeval(nvec);
    NumericVector x(nx);
    x = numerical_solution_inverseproblem(param, nx);
    xeval = observation_points_inverseproblem(nvec);
...
NumericVector observation_grad_val_inverseproblem(NumericVector param, int l, int m)
{
    ...
    pgrad[m] = 1.0;
    x = numerical_solution_derivative_inverseproblem(param, pgrad, nx);
    xeval = observation_points_inverseproblem(nvec);

    for(kvec = 0; kvec < nvec; kvec ++)
    {
        ind0 = (int)(xeval(kvec) / dh);
        w0 = (kvec + 0.5) / nvec / dh - ind0;
        xvec(kvec) = (1.0 - w0) * x[ind0] +  w0 * x[ind0 + 1];
    }
    return xvec;
}
...
NumericMatrix observation_grad_inverseproblem(NumericVector param, int l)
{
    ...
    for(k0 = 0; k0 < nparam; k0 ++)
    {
        pgrad = observation_grad_val_inverseproblem(param, l, k0);
        for(k1 = 0; k1 < nvec; k1 ++)
        {
