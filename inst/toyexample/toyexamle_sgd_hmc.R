...
# mixture of HMC and RWM kernels is utilized to estimate the gradient
    ...
    for(kiter in 1 : niter)
    {
        ...
        if(vlevel > 0)
        {
            x_increm = unbiased_increment(level = 0 + vlevel,
                                          ...
                                          h = function(l, x) objective_function(l, x),
                                          k = k0, m = m0,
                                          sampling_factor = min(0.1, 1.0 / (2.0 ^ (2 * vlevel + 1))),
                                          max_iterations = Inf)
        ...
        if((abs(gf * sgd_vec[kiter, 1]) * sm_param) > 0.5)
        {
            #print("high gradient correction")
            sgd_vec[kiter, 1] = 0.5 / sm_param / gf * sgd_vec[kiter, 1] / abs(sgd_vec[kiter, 1])
        }
        ...
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v1.rds")
        saveRDS(sigma_rep, file = fname)
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v1.npy")
        npySave(fname, sigma_rep)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v1.rds")
        saveRDS(sgd_vec, file = fname)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v1.npy")
        npySave(fname, sgd_vec)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v1.rds")
        saveRDS(nopr, file = fname)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v1.npy")
        npySave(fname, nopr)
    }
}
...
for(kreal in 1 : nreal)
{
    ...
    for(kiter in 1 : niter)
    {
        ...
        # record the data
        sigma_rep[kiter] = 0.0 + sm_param
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v2.rds")
        saveRDS(sigma_rep, file = fname)
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v2.npy")
        npySave(fname, sigma_rep)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v2.rds")
        saveRDS(sgd_vec, file = fname)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v2.npy")
        npySave(fname, sgd_vec)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v2.rds")
        saveRDS(nopr, file = fname)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v2.npy")
        npySave(fname, nopr)
...
for(kreal in 1 : nreal)
{
    ...
    for(kiter in 1 : niter)
    {
        ...
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v3.rds")
        saveRDS(sigma_rep, file = fname)
        fname = paste0("./toyexample_sgd_sigma_hmc_kreal_", kreal, "_v3.npy")
        npySave(fname, sigma_rep)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v3.rds")
        saveRDS(sgd_vec, file = fname)
        fname = paste0("./toyexample_sgd_hmc_nreal_", kreal, "_v3.npy")
        npySave(fname, sgd_vec)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v3.rds")
        saveRDS(nopr, file = fname)
        fname = paste0("./toyexample_nopr_sgd_hmc_nreal_", kreal, "_v3.npy")
        npySave(fname, nopr)
    }
}
