...
if(gf_case == 1)
{
    ...
    for(kreal in 1 : nreal)
    {
        ...
        for(kiter in 1 : niter)
        {
            ...
            if(vlevel == 0)
            {
                x_expect = unbiased_expectation(level = 3 + vlevel,
                                                rinit = rinit,
            ...
            if(vlevel > 0)
            {
                x_increm = unbiased_increment(level = 3 + vlevel,
                                              rinit = rinit,
                                              single_kernel = single_pcn_kernel,
            ...
            fname = paste0("./inverseproblem_sgd_acc_theta_pcn_kreal_", kreal, "_v1.rds")
            saveRDS(theta_rep, file = fname)
            fname = paste0("./inverseproblem_sgd_acc_theta_pcn_kreal_", kreal, "_v1.npy")
            npySave(fname, theta_rep)
            fname = paste0("./inverseproblem_sgd_acc_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./inverseproblem_sgd_acc_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, sgd_vec)
            fname = paste0("./inverseproblem_nopr_sgd_acc_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(nopr, file = fname)
            fname = paste0("./inverseproblem_nopr_sgd_acc_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, nopr)
