# This script generates data to perform MAP estimate of the hyperparameter with SGD
rm(list=ls())
library(Rcpp)
library(RcppCNPy)
...
if(gf_case == 1)
{
    gf = 0.10  # factor for the SGD step-size. SGD step size is gf / sgd_step
    xval = array(rep(0, nv, dim = c(nv)))
    ...
    for(kreal in 1 : nreal)
    {
        ...
        theta_rep <- array(rep(0, niter), dim = c(niter))  # array of theta values
        xval = array(rep(0, nv), dim = c(nv))
        ...
        for(kiter in 1 : niter)
        {
            ...
            if((abs(gf * sgd_vec[kiter, 1]) * theta_param) > 1.0)
            {
                #print("high gradient correction")
                sgd_vec[kiter, 1] = 1.0 / theta_param / gf * sgd_vec[kiter, 1] / abs(sgd_vec[kiter, 1])
            }
            # update the std in the likelihood function
            theta_param = theta_param * exp(gf / kiter * sgd_vec[kiter, 1] * theta_param)

            # record the data
            theta_rep[kiter] = 0.0 + theta_param

            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v1.rds")
            saveRDS(theta_rep, file = fname)
            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v1.npy")
            npySave(fname, theta_rep)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, sgd_vec)
            fname = paste0("./inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(nopr, file = fname)
            fname = paste0("./inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, nopr)
...
if(gf_case == 2)
{
    ...
    for(kreal in 1 : nreal)
    {
        ...
        for(kiter in 1 : niter)
        {
            ...
            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v2.rds")
            saveRDS(theta_rep, file = fname)
            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v2.npy")
            npySave(fname, theta_rep)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v2.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v2.npy")
            npySave(fname, sgd_vec)
            fname = paste0(".inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v2.rds")
            saveRDS(nopr, file = fname)
            fname = paste0("./inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v2.npy")
            npySave(fname, nopr)
...
if(gf_case == 3)
{
    ...
    for(kreal in 1 : nreal)
    {
        ...
        for(kiter in 1 : niter)
        {
            ...
            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v4.rds")
            saveRDS(theta_rep, file = fname)
            fname = paste0("./inverseproblem_sgd_theta_pcn_kreal_", kreal, "_v4.npy")
            npySave(fname, theta_rep)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v4.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./inverseproblem_sgd_pcn_nreal_", kreal, "_v4.npy")
            npySave(fname, sgd_vec)
            fname = paste0("./inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v4.rds")
            saveRDS(nopr, file = fname)
            fname = paste0("./inverseproblem_nopr_sgd_pcn_nreal_", kreal, "_v4.npy")
            npySave(fname, nopr)
