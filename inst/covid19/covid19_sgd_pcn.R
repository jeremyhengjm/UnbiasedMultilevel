...
if(gf_case == 1)
{
    ...
    for(kreal in 1 : nreal)
    {
        ...
        for(kiter in 1 : niter)
        {
            ...
            fname = paste0("./covid19_sgd_alpha_pcn_kreal_", kreal, "_v1.rds")
            saveRDS(alpha_rep, file = fname)
            fname = paste0("./covid19_sgd_alpha_pcn_kreal_", kreal, "_v1.npy")
            npySave(fname, alpha_rep)
            fname = paste0("./covid19_sgd_beta_pcn_kreal_", kreal, "_v1.rds")
            saveRDS(beta_rep, file = fname)
            fname = paste0("./covid19_sgd_beta_pcn_kreal_", kreal, "_v1.npy")
            npySave(fname, beta_rep)

            fname = paste0("./covid19_sgd_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./covid19_sgd_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, sgd_vec)
            fname = paste0("./covid19_nopr_sgd_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(nopr, file = fname)
            fname = paste0("./covid19_nopr_sgd_pcn_nreal_", kreal, "_v1.npy")
            npySave(fname, nopr)
        }
    }
}
