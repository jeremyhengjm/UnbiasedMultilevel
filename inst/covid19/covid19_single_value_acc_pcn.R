...
for(kreal in 1 : nreal)
{
    ...
    for(kiter in 1 : niter)
    {
        ...
        fname = paste0("./covid19_single_value_acc_pcn_nreal_", kreal, ".rds")
        saveRDS(sgd_vec, file = fname)
        fname = paste0("./covid19_single_value_acc_pcn_nreal_", kreal, ".npy")
        npySave(fname, sgd_vec)

        fname = paste0("./covid19_nopr_sv_acc_pcn_nreal_", kreal, ".rds")
        saveRDS(nopr, file = fname)
        fname = paste0("./covid19_nopr_sv_acc_pcn_nreal_", kreal, ".npy")
        npySave(fname, nopr)
