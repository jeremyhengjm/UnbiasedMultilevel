# This script generates compute the ratio of the real data to the expection for optimal parameters of gamma distribution
rm(list=ls())
library(Rcpp)
library(RcppCNPy)
                               ...
                               xdata = xdata_ref, umid = umid_param, sf = sf_param)
{
    ...
    nmax = 52
    xmeas = observation_covid19(ustar, l)  # model predictions
    xmetric = log(xmeas[nmin : nmax]) - 1.0 * log(xdata[nmin : nmax]) # argument of the Gamma distribution
...
nv = 24  # dimension of the paramet space (of the gradient in sgd)
nlevel = 5
...
if(gf_case == 1)
{
    ...
    for(kreal in 1 : nreal)
    {
        ...
        xval = array(rep(0, nv), dim = c(nv))
        alpha_param = 2.287787302358859  # shape of the gamma distribution
        beta_param = 1.207776176265537  # scale of the gamma distribution
        ...
        for(kiter in 1 : niter)
        {
            ...
            fname = paste0("./covid19_obs_pcn_nreal_", kreal, "_v1.rds")
            saveRDS(sgd_vec, file = fname)
            fname = paste0("./covid19_obs_pcn_nreal_", kreal, "_v1.npy")
